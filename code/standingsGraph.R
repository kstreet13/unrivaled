makeUnrivaledStandingsGraph <- function(year, sched, mode = c('light','dark')){
    mode <- match.arg(mode)
    ...
    curves <- lapply(teams, function(team){
        away <- sched[which(sched$team1 == team), ]
        away$WL <- away$score1 > away$score2
        team2 <- sched[which(sched$team2 == team), ]
        team2$WL <- team2$score2 > team2$score1
        curve <- rbind(away, team2)
    ...
    # lgnd <- merge(lgnd, standings, by.x = 'team', by.y = 'Team')
    ...
    plot(range(do.call(rbind, curves)$date), range(sapply(curves,function(x){x$margin})), col = 'transparent',
         xlab = 'Date', ylab='Games Over/Under .500', main = paste(year, 'Unrivaled Standings'), las = 1)
